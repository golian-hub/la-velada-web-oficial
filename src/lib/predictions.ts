...
const invalidateCache = (combatId?: string) => {
  if (combatId) {
    // Invalidar caché específica del combate y también la global para evitar datos stale
...
export async function getAllPredictions(): Promise<CombatPrediction[]> {
  ...
  try {
    ...
    result.rows.forEach((row) => {
      ...
      // Agregar predicción del luchador si no existe ya
      const existingIndex = combatPredictions[combatId].predictions.findIndex(
        (p) => p.fighter_id === fighterId,
      )
      if (existingIndex === -1) {
        combatPredictions[combatId].predictions.push({
  ...
  try {
    ...
    if (existingUserVote.rows.length > 0) {
      ...
      await turso.batch([
        ...
        {
          sql: `
            INSERT INTO predictions (combat_id, fighter_id, votes, updated_at)
            VALUES (?, ?, 1, CURRENT_TIMESTAMP)
            ON CONFLICT(combat_id, fighter_id) 
            DO UPDATE SET votes = votes + 1, updated_at = CURRENT_TIMESTAMP
          `,
          args: [combatId, fighterId],
        },
        {
